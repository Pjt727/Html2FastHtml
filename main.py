...
AUTO_FORMATTER = "ruff format"
PATH_TO_FILE = os.path.join("out", "fasthtml.py")
AUTO_FORMATTER_CMD = f"{AUTO_FORMATTER} {PATH_TO_FILE}"
...
@rt('/')
def get(): 
    return Title("Html2FastHtml"), Br(), Div(
            Form(
                Textarea(id="html_text", rows="20"),
                Button("To FastHtml", inputmode="submit"),
                hx_target="#html_text",
                hx_swap="outerHTML",
                hx_post="/toFastHtml",
                ),
...
copy_fast_html = '''
    var textarea = document.getElementById('html_text');
    textarea.select();
...
def post(html_text: str): 
    ...
    print(html_parser.fast_html_string)
    return Textarea(
            fast_html_text,
            id="html_text",
            rows="30"), Script(copy_fast_html)
