...
class FastHtmlParser(HTMLParser):
    ...
    def add_node_to_string(self, node: Node) -> None:
        ...
        py_attr_sequence = ",".join(
            [
                f"{py_attr}=None" if py_val is None else f"{py_attr}={py_val}"
    ...
    return (
        ...
        Div(
            Form(
                ...
                Input(id="automatic_copy", _type="checkbox", checked=True),
                Label(
                    "Use fastHTML parser",
                    _for="automaticCopy",
                    style="display: inline",
                ),
                Input(id="use_fasthtml_converter", _type="checkbox", checked=True),
                hx_target="#html_text",
                hx_swap="outerHTML",
                hx_post="/toFastHtml",
            ),
...
class ToFastHtmlRequest:
    ...
    use_fasthtml_converter: bool = False


@rt("/toFastHtml")
def post(fast_html_body: ToFastHtmlRequest):
    ft_text = ""
    if fast_html_body.use_fasthtml_converter:
        ft_text = html2ft(fast_html_body.html_text)
    else:
        html_parser = FastHtmlParser()
        html_parser.feed(fast_html_body.html_text)
        html_parser.generate_fast_html()
        ft_text = html_parser.fast_html_string
    ...
    return Textarea(fast_html_text, id="html_text", rows="20"), Script(script_contents)
